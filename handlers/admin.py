...
👥 Сообщение будет отправлено всем пользователям бота."""
            ...
            for i, user_key in enumerate(users.keys()):
                try:
                    await context.bot.send_message(
                        ...
                        text=message_text,
                        parse_mode='HTML',  # Изменено с Markdown на HTML для поддержки ссылок
    ...
    async def handle_media_broadcast(update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Обработчик медиа-рассылки (фото, видео, документы)"""
        user_id = update.effective_user.id
        
        if user_id not in ADMINS:
            return
        
        # Проверяем есть ли подпись с командой /broadcast
        ...
        for i, user_key in enumerate(users.keys()):
            ...
            if (i + 1) % 5 == 0:  # Для медиа реже обновляем статус
                try:
                    await status_msg.edit_text(f"📢 Рассылка медиа...\n✅ Успешно: {success_count}\n❌ Ошибок: {fail_count}")
                except:
                    pass
            
            await asyncio.sleep(0.15)  # Для медиа больше задержка
